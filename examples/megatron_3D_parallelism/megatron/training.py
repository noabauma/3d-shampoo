...
def train_step_pipe(model, data_iterator):
    ...
    print_rank_0('@@@@tokenize@@@@...')
    x = torch.randint(low=1, high=2, size=(args.batch_size,args.seq_length))
    t = torch.tensor([1] * args.batch_size)
    _dict = {'text': x, 'labels': t}
    batch = [_dict]*1#args.batch_size
