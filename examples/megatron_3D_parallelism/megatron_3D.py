...
from megatron.model import GPT2ModelPipe, GPT2Model
...
if __name__ == "__main__":
    ...
    # read ds_config.json file and get train_batch_size
    with open("ds_config.json", "r") as file:
        data = file.read()

        parsed_data = json.loads(data)

        train_batch_size = parsed_data["train_batch_size"]


    micro_batches = train_batch_size//(dp_dim*args.batch_size)
    print_rank_0(f"micro_batches: {micro_batches}")
    ...
    train_time = 0
    n_iters = 7 if not profile else 1
    for i in range(n_iters):
        if profile:
            ...
            print_rank_0(f"flops: {flops}")
            print_rank_0(f"params: {params}")
    ...
    print_rank_0(f"Throughput [tokens/s]: {micro_batches*args.batch_size*dp_dim/avg_train_time}")
    print_rank_0(f"avg_train_time [s]: {avg_train_time}")
    #print_rank_0(f"loss: {loss}")

    print_rank_0(f"avg_prec_time: {optimizer.prec_time},{np.mean(optimizer.prec_time[n_warmup:])},{np.std(optimizer.prec_time[n_warmup:])},{len(optimizer.prec_time)}")
    ...
    if write_files:
        if args.shampoo:
            ...
            for name, module in model.named_modules():
                if hasattr(module, 'weight'):
                    ...
                    if world_rank == comm_list_[partitioning[0]]:
                        cnt_precs += len(preconditioners[0])
                ...
                if hasattr(module, 'bias'):
                    ...
                    if world_rank == comm_list_[partitioning[0]]:
                        assert len(preconditioners[0]) == 0
            ...
            print_rank_0(f"number of precs of world_rank {world_rank}: {cnt_precs}")
        else:
